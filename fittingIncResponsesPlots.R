...
# plots results from predicted brand from data in SurveyIncomplete.csv 
...
# summary of data
# summary(incResponses$brand)

# visualisations ####
# Brand relative preference for complete survey
plotIncResponseBrandRelPreference <- ggplot(incResponses, aes(brand), color=brand) + 
  geom_bar(aes(y = (..count..)/sum(..count..))) + 
  scale_y_continuous(labels=scales::percent) +
  ggtitle("Brand preference for incomplete survey") +
  ylab("relative preference")

plotIncResponseBrandRelPreference

# boxplot brand vs age
qplot(age, brand, data=incResponses, geom = 'boxplot', color = brand)

# boxplot brand vs salary
qplot(salary, brand, data=incResponses, geom = 'boxplot', color = brand)
