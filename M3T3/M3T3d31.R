...
# -4- #
## __ select 5 highest RSSI values by row 
### write the 5 top values in 3 extra cols at the end of the DF -> trData.waps.4
max5byRow <- rbind.data.frame(apply(trData.waps.3, 1, function(x) cbind(x[order(x, decreasing=TRUE)[1:5]])))
trData.waps.4 <- cbind(trData.waps.3, t(max5byRow))
...
max5byRow <- rbind.data.frame(apply(vlData.waps.2, 1, function(x) cbind(x[order(x, decreasing=TRUE)[1:5]])))
vlData.waps.4 <- cbind(vlData.waps.2, t(max5byRow))
...
### transform log scale
base <- as.numeric(2^(1/6))
trData.waps.5 <- t(apply(trData.waps.5, 1, function(x){ return(as.integer(round(base^x))) }))
...
vlData.waps.5 <- t(apply(vlData.waps.5, 1, function(x){ return(as.integer(round(base^x))) }))
...
#   > postResample(trData.3pLat$predLat, trData.3pLat$LATITUDE)
# RMSE   Rsquared        MAE 
# 11.0487955  0.9752993  6.1459957 
# > postResample(trData.3pLon$predLon, trData.3pLon$LONGITUDE)
# RMSE   Rsquared        MAE 
# 15.9389774  0.9825113  7.0146554 
# > 
#   > diffLAT <- abs(trData.3pLat$LATITUDE - trData.3pLat$predLat)
# > diffLON <- abs(trData.3pLon$LONGITUDE - trData.3pLon$predLon)
# > 
