...
# sum(varCol < 250, na.rm=T) ## 400 (23 NA)
...
## keep only WAPS cols that have var > 150 -> varCut
### make an index
varCut <- 150
...
### train model: svmLinear(LATITUDE ~ .), data = trData.3m 
...
### train model: svmLinear(LONGITUDE ~ .), data = trData.3m 
start_time <- Sys.time()
LonSVM <- train(
  LONGITUDE ~ ., 
  data = trainData.3m,
...
start_time <- Sys.time()
vlData.p$predLat <- predict(LatSVM, vlData.p)
end_time <- Sys.time()
print(end_time - start_time)
beep();

start_time <- Sys.time()
vlData.p$predLon <- predict(LonSVM, vlData.p)
end_time <- Sys.time()
print(end_time - start_time)
beep();
...
hist(diffEUC, breaks=90, main='SVM , varCut=150')
