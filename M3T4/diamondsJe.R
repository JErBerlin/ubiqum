...
trData$rdPrice <- as.integer(round(log(trData$price)*25)-145)
...
### train model: knn(price ~ rdPrice), data = trainData
start_time <- Sys.time()
priceKNN <- train(
  price ~ rdPrice, 
  data = trainData,
...
hist(diffPrice, breaks=100, main='KNN(Price ~ .), k=5, use all vars AND rdPreis')

print("KNN(Price ~ .), k=5, use all vars AND rdPreis")
